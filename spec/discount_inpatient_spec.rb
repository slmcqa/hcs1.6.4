...
#    @@doctor_dependent = "1309047657"#"1211069307" # "1112100132"
    ...
    @@employee = "1006513839"
    @@employee_dependent = "1404069337"
    @@board_member ="1404069624"
    @@board_member_dependent = "1404069625"
    @@doctor ="1404069628"
    @@doctor_dependent = "1411077964"  #conflict with discount outpatient spec
...
#    sleep 3
#    if (slmc.get_text("//html/body/div/div[2]/div[2]/div[22]/table/tbody").gsub(' ', '').include?  @@employee_dependent) && slmc.is_element_present("link=Admit Patient")
...
####### #COMPANY###
  it "COMPANY - Creates Patient" do
    ...
    slmc.create_new_admission(:rch_code => "RCH07", :org_code => "0287", :diagnosis => "GASTRITIS", :account_class => "COMPANY", :guarantor_code => "ABSC001").should == "Patient admission details successfully saved."
  end
...
#   @@total_discount = ((@@discount) * 100).round.to_f / 100
  ...
  it "BOARD MEMBER - Creates Patient" do
    ...
    if (slmc.get_text("//html/body/div/div[2]/div[2]/div[22]/table/tbody").gsub(' ', '').include?  @@board_member) && slmc.is_element_present("link=Admit Patient")
...
################################# Doctor Dependent
  it "DOCTOR DEPENDENT - Creates Patient" do
    ...
    if (slmc.get_text("//html/body/div/div[2]/div[2]/div[22]/table/tbody").gsub(' ', '').include?  @@doctor_dependent) && slmc.is_element_present("link=Admit Patient")
...
################################# DOCTOR
  it "DOCTOR - Creates Patient" do
    ...
    if (slmc.get_text("//html/body/div/div[2]/div[2]/div[22]/table/tbody").gsub(' ', '').include?  @@doctor) && slmc.is_element_present("link=Admit Patient")
      slmc.create_new_admission(:rch_code => "RCH08", :org_code => "0287", :diagnosis => "GASTRITIS", :account_class => "DOCTOR", :guarantor_code => "6055").should == "Patient admission details successfully saved."
    else
      if slmc.is_text_present("NO PATIENT FOUND")
