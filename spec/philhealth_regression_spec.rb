...
describe "SLMC :: Regression of Issues for PhilHealth" do
  ...
  it "Bug #40494 - Contact ID : Philhealth" do
    ...
    sleep 3
    slmc.click("//input[@type='button' and @value='Preview']", :wait_for => :page)
...
##### NO ER PHILHEALTH
  ...
  it "Bug #40151 - [PH] SPU-DR: Exception error thrown after hitting PH Compute button" do
    ...
    @@ph2 = slmc.philhealth_computation(:claim_type => "REFUND", :diagnosis => "Z38.0", :medical_case_type => "ORDINARY CASE", :compute => true)
  ...
  it "Bug #40278 - [PH] SPU-DR: Patient PH claims not computed as normal case computation" do
    ...
    @@ph[:or_actual_lab_benefit_claim].should == ("%0.2f" %(@@actual_lab_benefit_claim))
    @@ph[:or_actual_lab_benefit_claim].should_not == ("%0.2f" %(1000))
    @@ph[:or_total_actual_benefit_claim].should_not == ("%0.2f" %(1000))
  end
