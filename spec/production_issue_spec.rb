...
it "2528 - PBA:Compensability Class - Yikes in Search by Discharged Date" do
    slmc.login("sel_pba6", @password).should be_true
    ...
    slmc.select("//html/body/div[4]/div/div/select[2]", "2011")
    slmc.click("link=12");
   # slmc.type("id=timeFrom", "01:00am");
    slmc.click("xpath=(//img[@alt='...'])[2]");
    slmc.select("//html/body/div[4]/div/div/select[2]","2013")
    slmc.click("link=14");
...
it "2584 - Pharmacy: Sales Invoice displays a 0.01 balance for Employees entitled to 100%discount" do
    ...
    slmc.oss_order(:item_code => "04999",:item_desc => "VIRLIX 10mg TAB",:order_add => true, :service_rate_display => "1439.82").should be_true
...
it "2535 - Admission:Can't update the patient info" do
   ...
   #   slmc.go_to_my_update_registration(:pin =>@@er_pin, :turn_inpatient => true, :save => true)
      ...
      sleep 6
      while slmc.is_text_present("1210127874") == false
          slmc.click("name=next");
          sleep 10
      end
      slmc.click("link=1210127874",:wait_for => :page)
...
it "2556 - PBA: Error encountered in reprinting OR" do
    ...
    slmc.type("id=cCheckDate", "5/20/2013");
...
#    slmc.click("//input[@type='submit' and @value='Proceed with Payment']")
    ...
    slmc.login("sel_pba6", @password).should be_true
    slmc.go_to_patient_billing_accounting_page
    slmc.pba_document_search(:select => "Payment", :doc_type => "OFFICIAL RECEIPT", :search_options => "DOCUMENT NUMBER",:entry => myor_no,:view_and_reprinting => true).should be_true
