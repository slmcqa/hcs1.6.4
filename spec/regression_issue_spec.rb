...
it "Bug#41063 - Checklist order: Re-include UOM" do
       ################### #OR SCENARIO
        slmc.login("sel_or5", @password).should be_true
       ...
       slmc.click "link=Home", :wait_for => :page
        ...
        while x != mycount
                if x == 1
                      status = slmc.get_text("//html/body/div/div[2]/div[2]/table/tbody/tr/td[7]")

                    else
                       status = slmc.get_text("//html/body/div/div[2]/div[2]/table/tbody/tr[#{x}]/td[7]")
                end
                if status == ""
                      if x== 1
                        #get pin
                          pin =  slmc.get_text("//html/body/div/div[2]/div[2]/table/tbody/tr/td[2]")
                          pin = pin.gsub(' ', '')
                      else
                           pin=    slmc.get_text("//html/body/div/div[2]/div[2]/table/tbody/tr[#{x}]/td[2]")
                             pin = pin.gsub(' ', '')
                      end
                      x = mycount
                    else
...
it "Feature #59377 - Order List modifications" do
  ...
  #  slmc.beep
    ...
    (slmc.get_text("//html/body/div/div[2]/div[2]/div[5]/ul/li[2]/a/span")).should == "PACKAGE"
...
it "Issue # 56018 - Newborn Admission: Add’l Patient Suffix field" do
       ...
       #   slmc.click("id=.save", :wait_for => :page);
          ...
          slmc.click("xpath=(//input[@name='action'])[2]",:wait_for => :page);
          slmc.is_text_present("Patient admission details successfully saved.")
         if selected_suffix == "JR"
          ...
          slmc.go_to_gu_page_for_a_given_pin("Order Page", babypin)
           slmc.get_tex('//*[@id="banner.fullName"]').include?(selected_suffix).should be_true
  end
...
it "53079	BIR Requirement: VAT computation on certain misc. types" do
   # slmc.login("sel_misc1", @password).should be_true
    ...
    mymisc_type = ["FOOD SUPPLIES","PHARMA SALES"]
...
it "1965 Enhancement FM - PACKAGE - PF amount should be available in Package rate" do
        slmc.login("fm", "fm").should be_true
        sleep 3
...
it "2057 FM: Service - Include NOW especially for items that are URGENT for updates. (eg. Consignment items)"do
        slmc.login("fm1", "fm").should be_true
        sleep 3
