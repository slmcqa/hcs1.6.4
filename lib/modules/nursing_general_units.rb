...
module NursingGenenalUnits
  ...
  def add_returned_order(options = {})
           ...
           #   end
              ...
              if options[:drugs]
                        ...
                        type("schedDate", (Date.today + 1).strftime("%m/%d/%Y"))
                        type("quantity", @quantity)
                        ...
                        click("gasContinuousFlag") if (options[:not_continous] == true && is_checked("gasContinuousFlag"))
                        type("gasHour", "12")
                        type("gasMinute", "30")
                        type("gasInterval", "10")
                        type("gasScheduleStartDateStr", start_date)
                        type("gasScheduleEndDateStr", end_date)
                        type("quantity", @quantity)
  ...
  def validate_orders(options ={})
    ...
    total_click = drugs_click + supplies_click + ancillary_click + others_click + special_click + procedures_click + non_procedures_click
    sleep 3
    return total_click
  end
  ...
  def confirm_validation_some_items
    click("validate")
  ...
  def er_print_gatepass(options={})
    ...
    if get_text("css=#occupancyList>tbody>tr>:nth-child(7)") == ("Discharged With Payment") || get_text("css=#occupancyList>tbody>tr>:nth-child(7)") == ("Discharged with Payment")
      click("btnPhysOut-#{options[:visit_no]}", :wait_for => :page)
      patient_pin_search options
      return true if is_text_present("NO PATIENT FOUND")
    else
      return false
  ...
  def print_gatepass(options ={})
    if is_text_present("NO PATIENT FOUND") == true
            ...
            if get_text("//html/body/div/div[2]/div[2]/table/tbody/tr/td[5]") == "Discharged With Payment"
                  click 'physout', :wait_for => :element, :element => 'submitPhysOut' if is_element_present('physout')
                  sleep 1
                  select 'roomBedStatusOptions', "label=GENERAL CLEANING"
                  click 'submitPhysOut', :wait_for => :page #General Cleaning
                  sleep 3
  ...
  def wellness_allocate_doctor_pf(options={})
    if options[:pf_type]
      click("allocatePf", :wait_for => :element, :element => "admDoctorRadioButton0")
      click("admDoctorRadioButton0")
      click("id=btnAddPf")
      sleep 3
