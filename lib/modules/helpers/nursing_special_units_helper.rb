...
#      click full_name_link if (options[:last_name] && (account_class == "EMPLOYEE") || (account_class == "EMPLOYEE DEPENDENT"))
  ...
  def admit_er_patient(options={})
    ...
    self.doctor_finder(:doctor => "ABAD")
     if options[:guarantor_code]
      ...
      sleep 3
      select "guarantorRelationCode","SELF"
    end
    click "previewAction", :wait_for => :page
  ...
  def click_outpatient_registration
...
#    select "id=userAction#{pin}", "label=#{page}"
  ...
  def add_returned_service(options = {})
    if options[:quantity]
      ...
      type 'aQuantity', qanaes
      type 'sQuantity', qsurg
      sleep 3
    end
  ...
  def confirm_order(options ={})
    type "anaesthDoctorCode", options[:anaesth_code]
    type "surgeonDoctorCode", options[:surgeon_code]
    type "surgeonDoctorCode", "3325" if ((CONFIG['location']) == 'QC' && options[:surgeon_code] == "6726")
    sleep 2
  ...
  def doctor_finder(options={})
    doctor_search = is_element_present("//input[@type='button' and @onclick=\"searchType='D';reinitDoctor();DF.show();\"]") ? "//input[@type='button' and @onclick=\"searchType='D';reinitDoctor();DF.show();\"]" : "//input[@type='button' and @onclick='reinitDoctor();DF.show();']"
